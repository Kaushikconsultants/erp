                    ...
                    <table className="logs-table">
                      ...
                      <tbody>
                        {webhookEvents.map((evt, i) => (
                          <tr key={i}>
                            <td className="log-phone">{evt.fromPhone}</td>
                            <td>{evt.customerName || <span className="log-muted">-</span>}</td>
                            <td><span className="log-code">{evt.type}</span></td>
                            <td><div className="log-truncate" title={evt.messageContent}>{evt.messageContent}</div></td>
                            <td><div className="log-truncate log-muted" style={{ maxWidth: '100px' }} title={evt.messageId}>{evt.messageId}</div></td>
                            <td><span className="log-badge badge-info">{evt.status}</span></td>
                            <td className="log-time">{formatTime(evt.timestamp)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
