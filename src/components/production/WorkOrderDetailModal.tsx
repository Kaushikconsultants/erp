...
export default function WorkOrderDetailModal({ workOrder, onClose, onPrint, onComplete, onUpdated }: Props) {
  ...
  const infoCard = (label: string, value: string | number, color?: string) => (
    <div style={{ backgroundColor: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "8px", padding: "10px 14px" }}>
      <div style={{ fontSize: "0.68rem", color: "#64748b", fontWeight: 700, marginBottom: "3px" }}>{label}</div>
      <div style={{ fontSize: "1rem", fontWeight: 800, color: color || "#0f172a", fontVariantNumeric: "tabular-nums" }}>{value}</div>
    </div>
  );
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <span style={{ fontSize: "1.05rem", fontWeight: 800, color: "#0f172a" }}>{workOrder.woNumber}</span>
                <span style={{ padding: "2px 7px", borderRadius: "5px", backgroundColor: "#dbeafe", color: "#2563eb", fontSize: "0.7rem", fontWeight: 700 }}>{workOrder.sector}</span>
                <span style={{ padding: "2px 7px", borderRadius: "5px", backgroundColor: "#fef3c7", color: "#d97706", fontSize: "0.7rem", fontWeight: 700 }}>{workOrder.priority} Priority</span>
              </div>
              <p style={{ margin: 0, fontSize: "0.78rem", color: "#475569", fontWeight: 600, marginTop: "1px" }}>{workOrder.finishedGoodsName} — {workOrder.title}</p>
            </div>
          </div>
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={onPrint} style={{ padding: "6px 12px", borderRadius: "7px", border: "1px solid #e2e8f0", backgroundColor: "#fff", color: "#475569", fontSize: "0.78rem", fontWeight: 700, cursor: "pointer" }}>🖨 Print Job Card</button>
            {workOrder.status !== "Completed" && workOrder.status !== "Cancelled" && (
              <button onClick={onComplete} style={{ padding: "6px 12px", borderRadius: "7px", border: "none", backgroundColor: "#059669", color: "#fff", fontSize: "0.78rem", fontWeight: 700, cursor: "pointer" }}>✅ Complete WO</button>
            )}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px" }}>
          ...
          <div style={{ marginBottom: "18px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.78rem", color: "#64748b", marginBottom: "6px" }}>
              <span style={{ fontWeight: 600 }}>Overall Production Progress</span>
              <span style={{ fontWeight: 800, color: "#4f46e5" }}>{progress.toFixed(1)}%</span>
            </div>
          ...
          {variantData && (
            <div style={{ backgroundColor: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "10px", padding: "14px", marginBottom: "16px" }}>
              <h4 style={{ margin: "0 0 10px", fontSize: "0.78rem", fontWeight: 800, color: "#1e40af" }}>📐 Size / Variant Breakdown</h4>
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                {Object.entries(variantData).map(([size, qty]: any) => (
                  <div key={size} style={{ backgroundColor: "#ffffff", border: "1px solid #bfdbfe", borderRadius: "8px", padding: "8px 14px", textAlign: "center", minWidth: "60px" }}>
                    <div style={{ fontSize: "0.7rem", color: "#64748b", fontWeight: 700 }}>{size}</div>
                    <div style={{ fontSize: "1.1rem", fontWeight: 900, color: "#1e40af", fontVariantNumeric: "tabular-nums" }}>{qty}</div>
                  </div>
                ))}
                <div style={{ backgroundColor: "#1e40af", border: "1px solid #1e40af", borderRadius: "8px", padding: "8px 14px", textAlign: "center", minWidth: "60px" }}>
                  <div style={{ fontSize: "0.7rem", color: "#93c5fd", fontWeight: 700 }}>TOTAL</div>
                  <div style={{ fontSize: "1.1rem", fontWeight: 900, color: "#ffffff", fontVariantNumeric: "tabular-nums" }}>
          ...
          {/* Stage Tracker */}
          <div style={{ marginBottom: "18px" }}>
            <h4 style={{ fontSize: "0.82rem", fontWeight: 800, color: "#0f172a", margin: "0 0 10px" }}>PRODUCTION STAGES</h4>
                ...
                return (
                  <div key={stage.id} style={{ border: `1.5px solid ${stage.status === "Completed" ? "#6ee7b7" : "#e2e8f0"}`, borderRadius: "10px", overflow: "hidden", backgroundColor: stage.status === "Completed" ? "#f0fdf4" : "#ffffff" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        <div style={{ width: "28px", height: "28px", borderRadius: "50%", backgroundColor: cfg.bg, border: `1.5px solid ${cfg.color}30`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: "0.75rem", color: cfg.color, flexShrink: 0 }}>{i + 1}</div>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: "0.84rem", color: "#0f172a" }}>{stage.stageName}</div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        <div style={{ display: "flex", gap: "16px", fontSize: "0.76rem" }}>
                          <span style={{ color: "#059669", fontWeight: 700 }}>✓ {stage.completedQty}</span>
                          <span style={{ color: "#dc2626", fontWeight: 700 }}>✗ {stage.rejectedQty}</span>
                        </div>
                        <span style={{ padding: "3px 8px", borderRadius: "5px", backgroundColor: cfg.bg, color: cfg.color, fontSize: "0.7rem", fontWeight: 700 }}>{stage.status}</span>
                        {stage.status !== "Completed" && (
                          ...
                          }} style={{ padding: "4px 8px", borderRadius: "5px", border: "1px solid #e2e8f0", backgroundColor: "#fff", color: "#2563eb", fontSize: "0.72rem", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: "3px" }}>
                    ...
                    {isEditing && (
                      <div style={{ borderTop: "1px solid #e2e8f0", padding: "12px 14px", backgroundColor: "#fafafa" }}>
                        <div style={{ display: "flex", gap: "10px", alignItems: "flex-end", flexWrap: "wrap" }}>
                          <div>
                            <label style={{ display: "block", fontSize: "0.68rem", fontWeight: 700, color: "#334155", marginBottom: "3px" }}>Completed Qty</label>
                            <input type="number" value={editCompletedQty} onChange={e => setEditCompletedQty(e.target.value)} style={{ width: "90px", height: "34px", padding: "0 8px", borderRadius: "6px", border: "1.5px solid #cbd5e1", fontSize: "0.84rem", textAlign: "center", fontWeight: 700 }} />
                          </div>
                          <div>
                            <label style={{ display: "block", fontSize: "0.68rem", fontWeight: 700, color: "#334155", marginBottom: "3px" }}>Rejected Qty</label>
                            <input type="number" value={editRejectedQty} onChange={e => setEditRejectedQty(e.target.value)} style={{ width: "90px", height: "34px", padding: "0 8px", borderRadius: "6px", border: "1.5px solid #fecdd3", fontSize: "0.84rem", textAlign: "center", fontWeight: 700, color: "#dc2626" }} />
                          </div>
                          <div>
                            <label style={{ display: "block", fontSize: "0.68rem", fontWeight: 700, color: "#334155", marginBottom: "3px" }}>Status</label>
                            <select value={editStatus} onChange={e => setEditStatus(e.target.value)} style={{ height: "34px", padding: "0 8px", borderRadius: "6px", border: "1.5px solid #cbd5e1", fontSize: "0.82rem", fontWeight: 700 }}>
                              {["In Progress","Completed","Skipped"].map(s => <option key={s}>{s}</option>)}
                            </select>
                          </div>
                          <div style={{ flex: 1, minWidth: "150px" }}>
                            <label style={{ display: "block", fontSize: "0.68rem", fontWeight: 700, color: "#334155", marginBottom: "3px" }}>Remarks</label>
                          ...
                          <div style={{ display: "flex", gap: "6px" }}>
                            <button onClick={() => setEditingStage(null)} style={{ padding: "6px 12px", borderRadius: "6px", border: "1px solid #e2e8f0", backgroundColor: "#fff", color: "#475569", fontSize: "0.78rem", fontWeight: 700, cursor: "pointer" }}>Cancel</button>
                            <button onClick={handleSaveStage} disabled={!!updatingStageId} style={{ padding: "6px 14px", borderRadius: "6px", border: "none", backgroundColor: "#059669", color: "#fff", fontSize: "0.78rem", fontWeight: 700, cursor: updatingStageId ? "wait" : "pointer" }}>
          ...
          {workOrder.materials.length > 0 && (
            <div style={{ marginBottom: "18px" }}>
              <h4 style={{ fontSize: "0.82rem", fontWeight: 800, color: "#0f172a", margin: "0 0 10px" }}>RAW MATERIALS / COMPONENTS</h4>
                  ...
                  <tbody>
                    {workOrder.materials.map((mat: any, i: number) => (
                      <tr key={i} style={{ borderBottom: "1px solid #f1f5f9" }}>
                        <td style={{ padding: "8px 10px", fontWeight: 600, color: "#0f172a" }}>{mat.materialName}</td>
                        ...
                        <td style={{ padding: "8px 10px", textAlign: "right", fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>₹{Number(mat.totalCost).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                        <td style={{ padding: "8px 10px" }}>
                          <span style={{ padding: "2px 6px", borderRadius: "4px", backgroundColor: "#f1f5f9", color: "#475569", fontSize: "0.68rem", fontWeight: 700 }}>{mat.status}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {workOrder.workerLogs.length > 0 && (
            <div>
              <h4 style={{ fontSize: "0.82rem", fontWeight: 800, color: "#0f172a", margin: "0 0 10px" }}>WORKER PRODUCTION LOGS ({workOrder.workerLogs.length})</h4>
              <div style={{ border: "1.5px solid #e2e8f0", borderRadius: "10px", overflow: "hidden" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.78rem" }}>
                  <thead style={{ background: "linear-gradient(180deg,#f8fafc,#f1f5f9)", borderBottom: "1.5px solid #e2e8f0" }}>
                    <tr>
                  ...
                  <tbody>
                    ...
                    <tr style={{ backgroundColor: "#faf5ff", borderTop: "1.5px solid #c4b5fd" }}>
                      <td colSpan={6} style={{ padding: "8px 10px", fontWeight: 800, color: "#7c3aed", textAlign: "right" }}>TOTAL WAGES PAYABLE:</td>
                      <td style={{ padding: "8px 10px", fontWeight: 900, color: "#7c3aed", textAlign: "right", fontVariantNumeric: "tabular-nums" }}>₹{totalWages.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
