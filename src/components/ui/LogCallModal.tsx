...
export default function LogCallModal({ 
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const isNotConnected = ["missed", "busy", "no answer", "rejected", "failed", "cancelled", "not connected", "voicemail", "callback", "call back"].some(
      s => selectedOutcome.toLowerCase().includes(s)
    );
    formData.set("durationSec", String(isNotConnected ? 0 : (durationSec || 0)));
              ...
              <select
                ...
                onChange={(e) => {
                  if (e.target.value === "__MANAGE_OUTCOMES__") {
                    ...
                    const isNotConn = ["missed", "busy", "no answer", "rejected", "failed", "cancelled", "not connected", "voicemail", "callback", "call back"].some(
                      s => val.toLowerCase().includes(s)
                    );
                    if (isNotConn) {
                      setDurationSec(0);
                    }
