...
async function runAudit() {
  ...
  for (const inv of invoices) {
    ...
    } else if (jvs.length > 1) {
      logIssue({
        id: `INV-DUP-JV-${inv.id}`,
        category: "Sales Accounting",
        severity: "CRITICAL",
        title: `Duplicate Journal Entries for Invoice (${inv.invoiceNumber})`,
        description: `Invoice ${inv.invoiceNumber} has ${jvs.length} posted journal entries.`,
        expected: "Exactly 1 JournalEntry",
        actual: `${jvs.length} JournalEntries found`,
        affectedTable: "Invoice",
        recordId: inv.id,
        financialImpact: `Duplicate revenue & debtor postings of ₹${inv.totalAmount * (jvs.length - 1)}`
      });
    } else {
      const jv = jvs[0];
  ...
  for (const p of payments) {
    const jvs = await prisma.journalEntry.findMany({
      where: {
        OR: [
          { sourceDocId: p.id, sourceDocType: "PAYMENT" },
  ...
  for (const b of bills) {
    const jvs = await prisma.journalEntry.findMany({
      where: {
        OR: [
          { sourceDocId: b.id, sourceDocType: "BILL" },
  ...
  for (const vp of vendorPayments) {
    const jvs = await prisma.journalEntry.findMany({
      where: {
        OR: [
          { sourceDocId: vp.id, sourceDocType: "VENDOR_PAYMENT" },
  ...
  for (const cn of creditNotes) {
    const jvs = await prisma.journalEntry.findMany({
      where: {
        OR: [
          { sourceDocId: cn.id, sourceDocType: "CREDIT_NOTE" },
